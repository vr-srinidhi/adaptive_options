...
class _ShadowLeg(NamedTuple):
    """One leg of a shadow-MTM reconstruction. `lots` is per-leg: a delta hedge
    can be smaller than the straddle it protects."""
    side: str
    option_type: str
    strike: int
    expiry_date: date
...
async def _compute_shadow_mtm(db: AsyncSession, run_row, legs) -> list:
    ...
    leg_info = [
        _ShadowLeg(l.side, l.option_type, l.strike, l.expiry_date, float(l.entry_price),
                   (int(l.quantity) // lot_size) if l.quantity else approved_lots)
        for l in legs if l.entry_price is not None
    ]
    charge_legs  = [(l.side, l.option_type, l.strike) for l in leg_info]
    entry_prices = [l.entry_price for l in leg_info]
    leg_lots     = [l.lots for l in leg_info]
    entry_charges = sum(
        compute_leg_entry_charges(lots, lot_size, [leg], [ep])
        for leg, ep, lots in zip(charge_legs, entry_prices, leg_lots)
    )
    ...
    for spot_row in spot_rows:
        ...
        gross_mtm_total = sum(
            ((l.entry_price - cur[(l.strike, l.option_type)])
             if l.side == "SELL" else
             (cur[(l.strike, l.option_type)] - l.entry_price))
            * lot_size * l.lots
            for l in leg_info
        )
        cur_prices = [cur[(l.strike, l.option_type)] for l in leg_info]
        est_exit = sum(
            compute_leg_exit_charges_estimate(lots, lot_size, [leg], [cp])
            for leg, cp, lots in zip(charge_legs, cur_prices, leg_lots)
        )
